...
import com.boc.lfj.httpdemo.bean.login.LoginResponseInfo;
...
public class SPUtils {
     ...
     *
     * @param spName      缓存的名称
     * @param keyValueMap 要存入缓存中的键值对,<String,
     ...
     * 删除登录信息
     */
    public static void deleteLoginInfoInSP() {
        deleteValueInSP(SPKeys.spName, SPKeys.cookie);
        deleteValueInSP(SPKeys.spName, SPKeys.uid);
        deleteValueInSP(SPKeys.spName, SPKeys.userid);
    }

    /**
     * 登录成功后，保存登录成功信息
     *
     * @param response 登录成功返回的bean类
     */
    public static void saveLoginState(LoginResponseInfo response) {
        SPUtils.setStringDataIntoSP(SPKeys.spName, SPKeys.userName, response.getUserName());
        SPUtils.setStringDataIntoSP(SPKeys.spName, SPKeys.uid, response.getUid());
        SPUtils.setStringDataIntoSP(SPKeys.spName, SPKeys.userid, response.getUserID());
    }

    /**
     * 获取本地保存的uid值
     *
     * @return 返回文件中存储的uid的值
     */
    public static String getUid() {
        return SPUtils.getStringValueFromSP(SPKeys.spName, SPKeys.uid);
    }
    ...
    public static void saveCookie(Response response) {
        ...
        if (!StringUtil.isNullOrEmpty(cookie)){
            ...
            SPUtils.setStringDataIntoSP(SPKeys.spName, SPKeys.cookie, cookie);
        }
    }
